...
def create_comparison_dataset(path="CarperAI/openai_summarize_comparisons", split="train"):
    ...
    for sample in tqdm(dataset):
        ...
        if chosen_summary == rejected_summary:
            continue
        if prompt + chosen_summary + rejected_summary in _cache: 
            continue
        # add sample into cache for redundancy detection
        _cache.add(prompt + chosen_summary + rejected_summary)
        ...
        pairs.append(pair)
    print(f"Collect {len(pairs)} comparison samples in total!")
    return pairs
...
if __name__ == "__main__":
    ...
    batch_size = 16
    dev_dataloader = DataLoader(dev_dataset, shuffle=False, batch_size=batch_size, collate_fn=DataCollatorReward())
    model.cuda()
    model.eval()
    model.half()
    correct = 0
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(dev_dataloader), total=len(dev_dataloader)):
            ...
            # if step % 100 == 0: print(f"step {step}: {consumed_samples}")
